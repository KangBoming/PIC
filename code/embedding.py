...
parser.add_argument('--model',type=str,default='esm2_t33_650M_UR50D',help='The protein language model you selected to extract protein sequence embedding.')
parser.add_argument('--output_dir',type=str,help='The save directory of protein sequence embedding.')
parser.add_argument('--toks_per_batch',type=int,default=10000)
...
def generate_fasta(data_path,save_path):
...
def extract_embedding(model, fasta_file,output_dir, toks_per_batch, 
                      repr_layers, device, include, truncation_seq_length):
    model, alphabet = pretrained.load_model_and_alphabet(model)
    model.eval()
...
def main():
    generate_fasta(data_path=args.data_path,save_path=args.fasta_file)
    extract_embedding(
        model=args.model,
